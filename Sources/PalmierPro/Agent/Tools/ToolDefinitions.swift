...
enum ToolDefinitions {
    static let all: [AgentTool] = [
        ...
        AgentTool(
            ...
            description: "Switches the active timeline — the one every read and edit tool targets and the one the user sees. get_media lists the project's timelines (with timelineId). Always re-read get_timeline after switching; clip and track ids from the previous timeline are no longer valid targets.\n\nTo edit the contents of a nested timeline (a clip with mediaType 'sequence'), switch to its mediaRef. Multicam groups open the same way for per-clip property edits (transform, crop, effects, color), splits, and deletes — but the time axis inside is locked: no moves, ripples, retimes, timing slips, or track changes. Angle cuts go through change_cam either way.",
            inputSchema: objectSchema(
                properties: [
        ...
        AgentTool(
            name: .syncClips,
            description: "Align one or more clips to a reference clip by shifting targets on the timeline — use for dual-system sound (camera + external audio) or multicam. Default mode 'auto' lets audio decide: cross-correlation seeded by capture dates when present; embedded source timecode only places targets audio can't (no audio, or no confident match). Mode 'timecode' trusts timecode outright (exact, confidence 1.0) for jam-synced rigs; force a method with mode. referenceClipId stays put unless a target would land before frame 0, in which case the whole group shifts right together (reported as shiftedFrames). Returns offsetFrames, confidence (0–1), and method (timecode|audio) per target; refuses weak audio matches.",
            inputSchema: objectSchema(
                properties: [
        ...
        AgentTool(
            name: .changeCam,
            description: "Switch which camera a multicam group shows, per frame range — batched entries, one undo step. Ranges are frames of the active timeline: project frames from an edit timeline, the group's own frames when it is the active timeline. Each entry is either a full-frame angle ({range, angle}) or a multi-angle layout ({range, layout, slots}) for split-screen and PiP moments. The group's timeline clip is never split or moved: angle cuts happen inside the group, the returned delta is usually empty, and track indices never shift — no re-read needed between calls. Works transparently across word-cut fragments, and after remove_words/remove_silence in either order.\n\nRanges where a chosen angle wasn't recording are clamped to what it covers (reported in clamped with the culprit) or skipped entirely; a skipped range leaves no edits. Adjacent same-angle cuts merge back into one (cutsMerged). Returns the resulting program as run-length [angle, startFrame, endFrame) rows over the touched span.",
            inputSchema: objectSchema(
                properties: [
